...
export default async function IocsPage() {
  ...
  return (
    <main className="overflow-y-auto flex flex-col gap-5 px-8 py-7" style={{ background: 'var(--bg)' }}>
      ...
      <div className="fade-up-2 rounded-[12px] overflow-hidden" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
        {iocs.length === 0 ? (
          ...
          <div style={{ borderBottom: '1px solid var(--border)' }}>
            <table className="w-full" style={{ borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  {['Type', 'Indicator', 'Verdict', 'Hits', 'Last seen'].map((h) => (
                    <th key={h} className="text-left px-5 py-3 font-mono text-[10px] font-medium tracking-[0.08em] uppercase" style={{ color: 'var(--muted)', borderBottom: '1px solid var(--border)' }}>
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {iocs.map((ioc, i) => (
                  <tr key={i} className="transition-colors duration-100 hover:bg-trion-surface2" style={{ borderBottom: i < iocs.length - 1 ? '1px solid var(--border)' : undefined }}>
